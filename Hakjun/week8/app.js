...
app.get('/users/:userId', (req, res) => { //동적입력주소는 콜론을 붙혀줌
...
app.put("/users/:userId", (req, res) => {
  ...
  if(id === -1){
    res.status(404).send("요청한 id가 없습니다.")
  }
  users[id] = {
    ...users[id],
    ...req.body
...
app.post("/users", (req,res) => {
  ...
  if(Object.keys(newUser).length === 0){
    res.status(400).send('user에 관한 정보 입력해주세요');
  }else if(Object.keys(newUser).length <4){
    res.status(400).send('user를 추가하기위해 필요한 정보 모두 입력해주세요')
  }else{
